	...
	initPCOLive: function() {
		...
		try {		
			if ((self.config.applicationid !== '') && (self.config.applicationid !== undefined) && (self.config.secretkey !== '') && (self.config.secretkey !== undefined)) {
				...
				self.doRest('GET', organization_url, {})
			...
			if (result.errors) {
				...
				if (currentItemId) {
					...
					if (item.type === 'ItemTime') {
						...
						let itemsWithoutHeader = items.filter(item => item.attributes.item_type !== 'header' && item.type === 'Item');
						...
						self.currentState.dynamicVariables['plan_currentitem_time_shouldfinish'] = dtTimeShouldFinish.toLocaleTimeString();
				...
				else { //no current item, so we must be at the beginning of the plan
					...
					let preServiceItems = items.filter(item => item.attributes.item_type !== 'header' && item.type === 'Item' && item.attributes.service_position == 'pre');
	...
	startPlanItemTimeRemainingInterval: function(dtTimeStarted, dtTimeShouldFinish) {
		...
		self.ITEM_TIME_REMAINING_INTERVAL = setInterval(function() {
			try {
				...
				let timeRemaining_formatted = Math.floor(Math.abs(timeRemaining) / 60) + ':' + ('0' + Math.floor(Math.abs(timeRemaining) % 60)).slice(-2);
				if (timeRemaining < 0) {
		...
		try {
			if (self.config.polling == true) {
				if (self.lastServiceTypeId && self.lastPlanId) {
					...
					self.log('error', 'Cannot start polling because we do not know what plan is being controlled yet. Start controlling a plan first.');
				}
			}
		}
		catch(error) {
